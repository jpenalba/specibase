...
| 6 | Auth & per-user roles (Owner/Lab member/Collaborator + RLS) | **In design** | Design doc: [`AUTH_AND_PERMISSIONS_PLAN.md`](./AUTH_AND_PERMISSIONS_PLAN.md) — a two-tier account model (lab member / collaborator) plus per-project Viewer/Editor/Owner roles, authorized in the existing API-route layer rather than a rearchitecture to RLS. Not started; open questions there need answers first. |
