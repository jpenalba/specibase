...
import { CustomValueUpsertInput, upsertCustomValues } from "@/lib/lab-workflows-store";
import { apiError } from "@/lib/api-error";

function parseValues(body: unknown): CustomValueUpsertInput[] | { error: string } {
  if (!Array.isArray(body) || body.length === 0) {
    return { error: "At least one value is required" };
  }
  const values: CustomValueUpsertInput[] = [];
...
// Bulk upsert, same shape as /api/lab-workflows/[id]/entries — one PATCH
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const result = await upsertCustomValues(values);
