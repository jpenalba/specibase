...
function parseEntries(body: unknown): BioEntryUpsertInput[] | { error: string } {
  ...
  const entries: BioEntryUpsertInput[] = [];
  for (const raw of body) {
    ...
    const entry: BioEntryUpsertInput = { step_id: row.step_id, sample_id: row.sample_id };

    if ("status" in row) {
      if (typeof row.status !== "string" || !ALL_STATUSES.includes(row.status as never)) {
        return { error: `Unknown status "${row.status}"` };
      }
      entry.status = row.status as BioEntryUpsertInput["status"];
    }
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const result = await upsertBioEntries(entries);
