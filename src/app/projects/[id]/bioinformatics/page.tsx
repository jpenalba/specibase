...
import { BIO_DETAIL_COLUMN_PRESETS } from "@/lib/bio-workflow-detail-columns";
...
const API_BASE = "/api/bio-workflows";

// Same shape as the Lab workflow tab (src/app/projects/[id]/lab-workflow/
// page.tsx) — every workflow for this project renders its own full grid
// here, stacked, and "New workflow" appends another section — just pointed
// at the Bioinformatic workflow's own endpoints and step/column vocabulary.
export default function ProjectBioinformaticsPage() {
  const { id: projectId } = useParams<{ id: string }>();

  const [workflows, setWorkflows] = useState<BioWorkflow[]>([]);
      ...
      {workflows.length === 0 ? (
        ...
        workflows.map((workflow) => (
          <WorkflowSection
            ...
            onDeleted={load}
            apiBase={API_BASE}
            stepPresets={BIO_STEP_PRESETS}
            detailColumnPresets={BIO_DETAIL_COLUMN_PRESETS}
          />
        ))
      )}
